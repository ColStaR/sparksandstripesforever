...
#ELEVATION
display(df_weather.select(col("HourlyPrecipitation").cast('int')))
df_weather.select([count(when(col("HourlyPrecipitation").cast('int').isNull(),True))]).show()

# Notes
# 88% missing, most data is zero, is this data at all useful? I vote no here I think
...
# right tailed
#No longer so tightly distributed
