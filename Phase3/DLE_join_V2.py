...
airport_weather = airport_weather.groupBy('ORIGIN','DATE') \
                                 .agg( *(avg(c).alias(c) for c in airport_weather.columns if c not in {'ORIGIN','DATE'}) )
...
df_final = df_airlines.join(airport_weather, "ORIGIN" )\
                      .filter((col("DEP_DATETIME_LAG") - expr("INTERVAL 2 HOURS") <= col("DATE")) & (col("DATE") < col("DEP_DATETIME_LAG")))\
                      .cache()
