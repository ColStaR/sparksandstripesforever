...
# MAGIC 5. Identify rows in the merged weather data with duplicated DATE and airport (ORIGIN). Address these duplicates by keeping the row with the smallest value in “distance_to_neighbor”. For any ties, just keep the first occurrence. This strategy allows us to keep more granular time in our weather data by making use of multiple stations. 
# MAGIC 6. Create a departure date-time column in the airlines dataset by combining the year, month, day, and departure time (CRS_DEP_TIME) into a datetime. Add a column to the airlines dataset that is equal to the departure datetime minus 2 hours. We will need to round the time down to the hour to help make merging onto the weather data easier, which can be accomplished by removing the last two digits from the departure time before converting to a datetime. 
# MAGIC 7. Finally, we can merge the airlines data and the weather data together as a one-sided (left) merge onto the airlines data using the columns: ORIGIN, DATE / CRS_DEP_TIME. The resulting table should have exactly as many rows as the original airlines dataset. 
# MAGIC 
# MAGIC With all three datasets merged, we would need to identify the number and proportion of flights that did not have any corresponding weather data. If this is a small enough set, we can drop rows that are missing weather data. If we find that we need to keep these rows, we can fill the missing values with averages for the given ORIGIN and MONTH. Any other important features with missing values can be handled similarly.
